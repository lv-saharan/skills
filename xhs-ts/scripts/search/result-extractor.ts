 ...
 * Uses Function constructor to avoid __name injection by tsx compiler
 * which conflicts with XHS page's own __name definition
 */
export async function extractSearchResults(page: Page, limit: number): Promise<SearchResultNote[]> {
  ...
  try {
    // Use Function constructor to create function at runtime
    // This bypasses tsx compiler's __name injection which conflicts with XHS page's __name
