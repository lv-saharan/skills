...
export async function extractSearchResults(page: Page, limit: number): Promise<SearchResultNote[]> {
  ...
  try {
    const containerSel = SEARCH_CONTAINER_SELECTOR;
    const noteSel = NOTE_ITEM_SELECTOR;
    const max = actualLimit;

    const rawData = await page.evaluate(
      async (opts: { containerSel: string; noteSel: string; max: number }) => {
        const { containerSel, noteSel, max } = opts;
        ...
        for (let idx = 0; idx < items.length && idx < max; idx++) {
          ...
          const cover = (imgEl as HTMLImageElement)?.src || '';
