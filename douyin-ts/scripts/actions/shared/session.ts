 ...
 * @description Unified session orchestration for all Douyin actions.
...
import { DouyinError, DouyinErrorCode } from '../../core/error';
import { config, DY_URLS } from '../../config';
...
import { getTmpFilePath } from '../../config/config';
 ...
 *
 * This is the PRIMARY API for all Douyin actions.
...
export async function withSession<T>(
  ...
  return withProfile(
    ...
    async (page, profileResult) => {
      ...
      if (navigateHome) {
        await page.goto(DY_URLS.home, { waitUntil: 'domcontentloaded', timeout: TIMEOUTS.pageLoad });
...
export async function checkPageHealth(page: Page): Promise<string | null> {
  ...
  if (pageContent.includes('页面不存在') || pageContent.includes('404')) {
    return '页面不存在';
  }
  if (pageContent.includes('访问受限')) {
    return '访问受限';
  }
...
export async function ensureLoginStatus(
  ...
  try {
    ...
    await page.waitForLoadState('networkidle', { timeout: TIMEOUTS.networkIdle }).catch(() => {});
    await delay(1500); // Extra buffer for dynamic content
    ...
    if (loginModalVisible) {
      debugLog('Login modal found -> waiting for scan');
      return {
        isLoggedIn: false,
        loginModalOpen: true,
      };
    ...
    if (userComponentVisible) {
      debugLog('User component found -> logged in');
      return { isLoggedIn: true };
    }

    // URL-based login detection
    const isOnHomePage = currentUrl === DY_URLS.home || currentUrl.startsWith(DY_URLS.home + '/');
    const isOnLoginPage = currentUrl.includes('/login');

    if (isOnHomePage && !isOnLoginPage && !loginModalVisible) {
      // Check if we can see video content (more reliable indicator)
      const hasVideoContent = await page.locator('a[href*="/video/"]').first().isVisible().catch(() => false);
      if (hasVideoContent) {
        debugLog('Video content visible -> assuming logged in');
        return { isLoggedIn: true };
      }
    }
    ...
    await page.goto(DY_URLS.login, { waitUntil: 'domcontentloaded', timeout: TIMEOUTS.pageLoad });
    await page.waitForLoadState('networkidle', { timeout: TIMEOUTS.networkIdle }).catch(() => {});
    await delay(1000);
...
// Re-export randomStealthDelay from browser-launcher
export { randomStealthDelay } from './browser-launcher';
