...
export async function checkPageHealth(page: Page): Promise<PageHealthStatus> {
  ...
  const errorResult = checkErrorPage(page);
  ...
  if (pageContent.includes('当前视频暂时无法浏览') || pageContent.includes('页面不见了')) {
    return {
      isHealthy: false,
      isLoggedIn: true,
      hasCaptcha: false,
 ...
 *
 * @param page - Playwright page
 * @param context - Context for better error messages ('video', 'user', 'general')
 * @returns Error message if found, null otherwise
 */
export async function checkContentErrors(
  page: Page,
  context: 'video' | 'user' | 'general' = 'general'
): Promise<string | null> {
  const content = await page.content();

  // Generic error messages (apply to all contexts)
  if (content.includes('当前视频暂时无法浏览') || content.includes('页面不见了')) {
    return context === 'video' ? '视频不可访问' : '页面不可访问';
  }
  if (content.includes('用户不存在')) {
    return '用户不存在';
  }
  if (content.includes('内容不存在')) {
    return context === 'video' ? '视频不存在' : '内容不存在';
  }
  if (content.includes('该内容因违规无法查看')) {
    return context === 'video' ? '视频因违规无法查看' : '内容因违规无法查看';
  }
