...
/**
 * Check if user is logged in on Douyin
 *
 * 检测顺序（优先级从高到低）：
 * 1. 用户头像 → 有 = 已登录
 * 2. 登录弹窗/登录按钮 → 有 = 未登录
...
export async function checkLoginStatus(page: Page): Promise<boolean> {
  try {
    ...
    // 优先级 4: 检查是否有视频内容（首页登录后才有）
    if (currentUrl.includes('douyin.com') && !currentUrl.includes('/login')) {
      const videoVisible = await page
        .locator('video, [class*="video-player"]')
        .first()
        .isVisible()
        .catch(() => false);

      if (videoVisible) {
        debugLog('Video content found → IS logged in');
        return true;
      }
    }
